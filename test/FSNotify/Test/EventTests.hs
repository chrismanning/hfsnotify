...
eventTests' testFolderGenerator threadingMode poll recursive nested = do
  ...
  when ((isFreeBSD || isLinux) && not poll && (recursive || not nested)) $ do
    it "renames directory" $ withFolder $ \(TestFolderContext _watchedDir f getEvents clearEvents) -> do
      liftIO $ do
        createDirectory f
        clearEvents
  ...
  when ((isFreeBSD || isLinux) && not poll && (recursive || not nested)) $ do
    it "renames file" $ withFolder $ \(TestFolderContext _watchedDir f getEvents clearEvents) -> do
      liftIO $ do
        writeFile f ""
        clearEvents
        renameFile f (init f)
  ...
  when ((isFreeBSD || isLinux) && not poll && (recursive || not nested)) $ it "renames directory outside watched dir" $ withFolder $ \(TestFolderContext _watchedDir f getEvents clearEvents) -> do
    withRandomTempDirectory testFolderGenerator $ \tmpDir -> do
      liftIO $ do
        createDirectory f
        clearEvents
  ...
  when ((isFreeBSD || isLinux) && not poll && (recursive || not nested)) $ it "renames file outside watched dir" $ withFolder $ \(TestFolderContext _watchedDir f getEvents clearEvents) -> do
    withRandomTempDirectory testFolderGenerator $ \tmpDir -> do
      liftIO $ do
        writeFile f ""
        clearEvents
