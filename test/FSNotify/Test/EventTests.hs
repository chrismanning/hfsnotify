    ...
    forM_ [False, True] $ \recursive -> describe (if recursive then "Recursive" else "Non-recursive") $
      forM_ [False, True] $ \nested -> describe (if nested then "In a subdirectory" else "Right here") $
        makeTestFolder threadingMode poll recursive nested $ do
          ...
          when (isFreeBSD && not poll) $ it "renames directory" $ \(_watchedDir, f, getEvents, clearEvents) -> do
            createDirectory f >> clearEvents
          ...
          when (isFreeBSD && not poll) $ it "renames file" $ \(_watchedDir, f, getEvents, clearEvents) -> do
            writeFile f "" >> clearEvents
            renameFile f (init f)
          ...
          when (isFreeBSD && not poll) $ it "renames directory outside watched dir" $ \(_watchedDir, f, getEvents, clearEvents) -> do
          ...
          when (isFreeBSD && not poll) $ it "renames file outside watched dir" $ \(_watchedDir, f, getEvents, clearEvents) -> do
            withRandomTempDirectory $ \tmpDir -> do
              writeFile f "" >> clearEvents
